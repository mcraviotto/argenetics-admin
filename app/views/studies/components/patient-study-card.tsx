...
export default function PatientStudyCard({ study }: { study: ListStudy }) {
  const studyTitle = study_options.flatMap(category => category.items).find(s => s.value === study.title)?.label || study.title
  ...
  return (
    <div className="flex flex-col bg-background p-4 rounded-md shadow-md h-[148px]">
      <div className="flex justify-between">
        <span className="text-md font-medium">
          {studyTitle}
        </span>
        <Badge
          className={cn("shadow-none rounded-full",
