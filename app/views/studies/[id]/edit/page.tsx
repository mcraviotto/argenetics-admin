...
const editStudySchema = newStudySchema
  ...
  .extend({
    ...
    result: z
      ...
      }, { required_error: "El resultado es requerido", invalid_type_error: "El resultado es requerido" })
      .refine((data) => data.storage_ref || data.file, {
...
export default function EditStudyPage() {
  ...
  const form = useForm<z.infer<typeof editStudySchema>>({
  ...
  async function onSubmit(values: z.infer<typeof editStudySchema>) {
    try {
      const { result, medical_order, additional_docs, study_category_id, ...data } = values

      const result_storage_ref = result.storage_ref ? result.storage_ref : (result.file ? await uploadFileToS3({ file: result.file, patient_id: values.patient_id }).unwrap() : "");
      const medical_order_storage_ref = medical_order?.storage_ref ? medical_order.storage_ref : (medical_order?.file ? await uploadFileToS3({ file: medical_order.file, patient_id: values.patient_id }).unwrap() : "");
      const additional_docs_storage_ref = additional_docs?.storage_ref ? additional_docs.storage_ref : (additional_docs?.file ? await uploadFileToS3({ file: additional_docs.file, patient_id: values.patient_id }).unwrap() : "");
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      patient_id: study?.patient?.id || "",
      doctor_id: study?.doctor?.id || "",
      study_category_id: study_options.find(category => category.items.some(item => item.value === study.title))?.value,
      state: study?.state,
      result: {
                  ...
                  {(!!medical_order?.file || medical_order?.storage_ref) ? (
                    <div className="flex items-center gap-2 p-2 pl-3 pr-4 rounded-md border transition-border justify-between shadow-sm hover:ring-1 ring-ring/50 transition-all">
                      ...
                      <Button
                        ...
                        size="icon"
                        onClick={() => {
                          form.setValue("medical_order", { file: undefined, storage_ref: "", file_name: "" }, { shouldValidate: true })
                        }}
                      >
                        <X />
                      </Button>
                    ...
                    <FormControl>
                      <FileUploader
                        id="medical_order"
                        onChange={(file) => {
                          form.setValue("medical_order", { file, storage_ref: "", file_name: file.name }, { shouldValidate: true })
                        }} />
                    </FormControl>
                  }
                  <FormMessage />
                  ...
                  {(!!result?.file || result?.storage_ref) ? (
                    <div className="flex items-center gap-2 p-2 pl-3 pr-4 rounded-md border transition-border justify-between shadow-sm hover:ring-1 ring-ring/50 transition-all">
                      ...
                      <Button
                        ...
                        size="icon"
                        onClick={() => {
                          form.setValue("result", { file: undefined, storage_ref: "", file_name: "" }, { shouldValidate: true })
                        }}
                      >
                        <X />
                      </Button>
                    ...
                    <FormControl>
                      <FileUploader
                        id="result"
                        onChange={(file) => {
                          form.setValue("result", { file, storage_ref: "", file_name: file.name }, { shouldValidate: true })
                        }} />
                    </FormControl>
                  }
                  <FormMessage />
                    ...
                    {(!!additional_docs?.file || additional_docs?.storage_ref) ? (
                      <div className="flex items-center gap-2 p-2 pl-3 pr-4 rounded-md border transition-border justify-between shadow-sm hover:ring-1 ring-ring/50 transition-all">
                        ...
                        <Button
                          ...
                          size="icon"
                          onClick={() => {
                            form.setValue("additional_docs", { file: undefined, storage_ref: "", file_name: "" }, { shouldValidate: true })
                          }}
                        >
                          <X />
                        </Button>
                      ...
                      <FormControl>
                        <FileUploader
                          id="additional_docs"
                          onChange={(file) => {
                            form.setValue("additional_docs", { file, storage_ref: "", file_name: file.name }, { shouldValidate: true })
                          }} />
                      </FormControl>
                    }
                    <FormMessage />
